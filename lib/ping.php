...
class Net_Ping
{
	...
	function ping_udp() {
        ...
        /* gert sane-function added in hostname retrieval */
	function ping_tcp() {
		/* Host must be nonblank */
		if ($this->host["hostname"]) {
			/* initialize variables */
			...
			/* determine the host's ip address */
			if ($this->is_ipaddress($this->host["hostname"])) {
				$host_ip = sane($this->host["hostname"]);
			}else{
				$host_ip = gethostbyname(sane($this->host["hostname"]));
			...
			while (1) {
				...
				case 2:
					...
					return false ; /* - gert -sure, host is alive, but it's a device check, meaning service check, I'm not interested in host alive... */
